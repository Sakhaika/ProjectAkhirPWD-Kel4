...
const DB = {
  ...
  seed() {
    ...
    const jalur = [
      {
        id: 'senaru',
        nama: 'Jalur Senaru',
        deskripsi: 'Jalur klasik dari sisi barat, melewati hutan tropis lebat dan pemandangan air terjun Sendang Gile.',
        ketinggian: 601,
        jarak: '18 km',
      ...
      },
      {
        id: 'sembalun',
        nama: 'Jalur Sembalun',
        deskripsi: 'Jalur dari sisi timur melalui padang savana luas, lebih panjang namun pemandangan lebih terbuka.',
        ketinggian: 1156,
        jarak: '22 km',
      ...
      },
      {
        id: 'torean',
        nama: 'Jalur Torean',
        deskripsi: 'Jalur terpendek namun paling menantang, melewati ngarai curam dan sumber air panas alami.',
        ketinggian: 600,
        jarak: '14 km',
        estimasi: '2 hari 1 malam',
        tingkat: 'Sangat Sulit',
        kuota_harian: 15,
        gambar: 'https://images.unsplash.com/photo-1551632811-561732d1e306?w=800&q=80'
      }
    ];
    ...
    const destinasi = [
      {
        ...
        deskripsi: 'Danau kaldera berwarna biru kehijauan di ketinggian 2.008 mdpl, salah satu danau terindah di Indonesia.',
      ...
      },
      {
        ...
        kategori: 'Air Terjun',
        deskripsi: 'Air terjun bertingkat dua dengan air jernih segar, terletak di kaki Rinjani dekat Desa Senaru.',
        lokasi: 'Desa Senaru, Lombok Utara',
        tiket: 'Rp 15.000 / orang',
        gambar: 'https://images.unsplash.com/photo-1467890947394-8171244e5410?w=800&q=80',
        rating: 4.7
      },
      {
        id: 'gili-trawangan',
        ...
        deskripsi: 'Pulau kecil dengan pantai berpasir putih, snorkeling, dan suasana santai khas Lombok.',
        lokasi: 'Lombok Utara',
      ...
      },
      {
        ...
        deskripsi: 'Bukit dengan hamparan sawah terasering yang cantik, spot foto ikonik dengan latar Rinjani.',
    ...
    ['senaru', 'sembalun', 'torean'].forEach(jalurId => {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        // Random terisi sebagian untuk demo
        const terisi = i < 3 ? kapasitas : Math.floor(Math.random() * (kapasitas * 0.6));
  ...
  // Jalur
  getJalur() { return JSON.parse(localStorage.getItem('db_jalur') || '[]'); },
  getJalurById(id) { return this.getJalur().find(j => j.id === id); },

  // Destinasi
  getDestinasi() { return JSON.parse(localStorage.getItem('db_destinasi') || '[]'); },

  // Kuota
  getKuota() { return JSON.parse(localStorage.getItem('db_kuota') || '{}'); },
  ...
  // Pendaftaran
  getPendaftaran() { return JSON.parse(localStorage.getItem('db_pendaftaran') || '[]'); },
  tambahPendaftaran(data) {
    const list = this.getPendaftaran();
    const id = 'SIMAKSI-' + Date.now();
    const newEntry = { ...data, id, status: 'pending', createdAt: new Date().toISOString() };
    list.push(newEntry);
  ...
  // Session
  setSession(admin) { sessionStorage.setItem('admin_session', JSON.stringify(admin)); },
  getSession() { return JSON.parse(sessionStorage.getItem('admin_session') || 'null'); },
  logout() { sessionStorage.removeItem('admin_session'); }
};
